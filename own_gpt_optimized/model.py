...
class GPTModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        x = checkpoint_sequential(self.blocks, segments=2, input=x, use_reentrant=False)
        x = self.ln_f(x)
        logits = self.head(x)
    ...
    def quantize_for_inference(self):
        """Dynamically quantize the model for CPU inference."""
        print("Quantizing model for inference...")
        return torch.quantization.quantize_dynamic(
            self.cpu(), {nn.Linear}, dtype=torch.qint8
        )